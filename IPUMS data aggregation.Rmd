...
lfpfemD <- filter(x,SEX==2 & EMPSTATD%in%c(100:120,140:240) & AGE%in%c(16:65)) %>%
  ...
  clf <- data.frame(tot.popn[,1:4],clfXX=NA);unempl <- data.frame(tot.popn[,1:4],unempXX=NA);
...
fwrite(orig.table,paste0(dsn,"/IPUMS_geodata_pct_11Nov17.csv")) # original sum table with the original GEOL
...
summary(ipums)
table(ipums$COUNTRY,ipums$YEAR)
ipums1 <- ipums %>%
  group_by()
