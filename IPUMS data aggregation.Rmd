...
#--Education                                    
  # number of persons with hs degree or less (Secondary eduction)
  ...
  lit25N <- filter(c1,LIT==2 & AGE%in%c(25:100)) %>% # ages 25+ yrs
    group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
    summarize(lit25NXX = sum(PERWT))
  
  # Percent of persons age 10+ literate (denominator)"
  lit25D <- filter(c1,LIT%in%c(1:2) & AGE%in%c(25:100)) %>% # ages 25+ yrs
    group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
    summarize(lit25DXX = sum(PERWT))
  
  educ <- tot.popn[join.names] %>% 
    left_join(educsec,by = join.names) %>% 
    left_join(educter,by = join.names) %>% 
    left_join(educD,by = join.names) %>% 
    left_join(lit25N,by = join.names) %>% 
    left_join(lit25D,by = join.names)
...
# Percent of currently employed persons age 16 to 65 who are working in agriculture, fishing or forestry
...
# Washer ownership
washerN <- filter(c1,WASHER==2 & GQ==10 & PERNUM==1) %>%
  group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
  summarize(washerNXX = sum(HHWT))
washerD <- filter(c1,WASHER%in%c(1:2) & GQ==10 & PERNUM==1) %>%
  group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
...
# Auto ownership
autoN <- filter(c1,AUTOS%in%c(20:43) & GQ==10 & PERNUM==1) %>%
  group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
  summarize(autoNXX = sum(HHWT))
autoD <- filter(c1,AUTOS%in%c(10:43) & GQ==10 & PERNUM==1) %>%
  group_by(COUNTRY,YEAR,GEOLEV1,GEOLEV2) %>%
